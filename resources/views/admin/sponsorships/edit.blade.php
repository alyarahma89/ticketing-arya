...
<head>
    ...
    <title>Edit Paket Sponsor - ARTIX ID</title>
        ...
        <header class="h-20 px-8 flex items-center justify-end gap-3 shrink-0 bg-white/50 backdrop-blur-md border-b border-slate-200/50 sticky top-0 z-50">
            <div class="flex items-center gap-3 bg-white px-4 py-2 rounded-full border border-slate-200 shadow-sm">
                ...
                <div class="flex flex-col text-left">
                    ...
                    <span class="text-[10px] text-artix-blue font-bold uppercase tracking-wider">Sponsor Editor</span>
                </div>
            </div>
        </header>
                ...
                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div>
                        ...
                        <h1 class="text-3xl font-black tracking-tight font-montserrat text-slate-900 mb-2 flex items-center gap-3">
                            Modifikasi Paket Sponsor
                            <span class="text-[12px] font-bold font-exo text-[#0066FF] bg-blue-50 px-3 py-1 border border-blue-200 rounded-lg shadow-sm">
                                ID: #{{ $sponsor->id }}
                            </span>
                        </h1>
                        <p class="text-[14px] text-slate-500 font-medium max-w-xl">Sesuaikan nilai valuasi, kuota maksimal pendanaan, dan klausa benefit mitra kemitraan ini.</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <form action="{{ route('admin.sponsorships.update', $sponsor->id) }}" method="POST" enctype="multipart/form-data">
                ...
                @if ($errors->any())
                    <div class="bg-red-50 border border-red-200 text-red-600 p-5 rounded-2xl mb-6 text-sm flex gap-3 items-start shadow-sm font-medium">
                        ...
                        <div>
                            <span class="font-bold block mb-1 text-red-700">Gagal memperbarui paket sponsor:</span>
                        ...
                        <div class="bg-white border border-slate-200 rounded-[24px] shadow-sm p-8 space-y-6 relative overflow-hidden">
                            ...
                            <div>
                                ...
                                <textarea name="benefits" rows="6" required placeholder="Contoh: Eksklusivitas logo di panggung utama, Space booth ukuran 4x4 meter..."
                                    class="w-full bg-slate-50 focus:bg-white border border-slate-200 text-slate-900 text-sm font-medium rounded-[12px] px-4 py-3.5 focus:outline-none focus:border-[#0066FF] focus:ring-4 focus:ring-[#0066FF]/10 transition-all duration-200 placeholder:text-slate-400">{{ old('benefits', $sponsor->benefits) }}</textarea>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white border border-slate-200 rounded-[24px] shadow-sm p-6 space-y-6 relative overflow-hidden">
                            ...
                            <div>
                                ...
                                <span class="text-[11px] text-slate-500 font-medium mt-2 block leading-relaxed">Jumlah korporat maksimal yang diizinkan untuk menandatangani paket ini.</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-white border border-slate-200 rounded-[24px] shadow-sm p-6 space-y-4">
                            ...
                            <div class="relative group aspect-square w-full max-w-[220px] mx-auto rounded-2xl bg-slate-50 border-2 border-dashed border-slate-300 overflow-hidden flex items-center justify-center shadow-inner transition-colors hover:border-[#0066FF]/50">
                                @if($sponsor->image)
                                    <!-- Preview jika sudah ada gambar di database -->
                                    ...
                                    <!-- Elemen placeholder disembunyikan secara default -->
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-slate-400 font-medium mt-2 text-center">Biarkan kosong jika tidak ingin mengubah gambar.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 bg-white border border-slate-200 rounded-[24px] p-6 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4 sticky bottom-6 z-40">
                    <span class="text-[13px] text-slate-500 font-medium flex items-center gap-2">
                        <i data-lucide="info" class="w-5 h-5 text-[#0066FF]"></i> Penyesuaian data tidak akan mengganggu riwayat transaksi yang sudah berjalan.
                    </span>
                    <div class="flex items-center gap-4 w-full sm:w-auto shrink-0">
                        ...
                        <button type="submit" class="bg-gradient-to-r from-[#0066FF] to-[#00C2FF] hover:opacity-90 text-white px-8 py-3.5 rounded-[12px] text-sm font-bold font-montserrat transition-all shadow-lg shadow-blue-500/30 hover:-translate-y-0.5 active:translate-y-0 w-full sm:w-auto text-center flex items-center justify-center gap-2">
                            Terapkan Perubahan <i data-lucide="check-circle" class="w-4 h-4"></i>
                        </button>
                    </div>
                </div>
            </form>
            ...
            badgeInput.addEventListener('change', function() {
                ...
                if(file) {
                    ...
                    reader.addEventListener('load', function() {
                        // Sembunyikan placeholder, tampilkan gambar & overlay
                    ...
                    // Jika file dibatalkan/dihapus dari input (opsional fallback)
                    // Tidak mereset ke gambar lama secara otomatis di UI ini untuk menghindari kebingungan
                }
            });
        }
    </script>
